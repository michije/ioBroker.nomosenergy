...
class Nomosenergy extends utils.Adapter {
    ...
    async storePrices(priceData) {
        ...
        // Create chart data for Flex Chart
        await this.setObjectNotExistsAsync("prices.chart_data", {
            type: "state",
            common: {
                name: "Chart data for prices",
                type: "string",
                role: "json",
                read: true,
                write: false,
        ...
        for (let i = 0; i < 48; i++) { // 48 hours (today + tomorrow)
            ...
            prices.push(matchingItem ? matchingItem.amount : null); // Use null if no price
            options.push({ xAxis: "" });
        }

        const chartData = JSON.stringify([labels, prices, options]);
        await this.setStateAsync("prices.chart_data", chartData, true);
    }
}
