...
async storePrices(priceData) {
    ...
    // Existing code to create folders and store individual prices...
    // (Assuming this part exists and remains unchanged)
    // Example: await this.setObjectNotExistsAsync("prices.today", {...});
    // Process priceData.items into today/tomorrow states...

    // Generate xAxis.data and series.data
    const xAxisData = [];
    const seriesData = [];
    for (let i = 0; i <= 48; i++) { // 49 points: today 00:00 to day after tomorrow 00:00
        const currentDate = new Date(today.getTime() + i * 3600000); // Add hours in milliseconds
        ...
        const matchingItem = priceData.items.find(item => {
